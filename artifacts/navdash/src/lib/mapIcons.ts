...
export function makeAISIcon(opts: {
  ...
  const { color, headingDeg, cogDeg, selected, label } = opts;
  const rot = headingDeg ?? cogDeg ?? 0;
  const hasDir = headingDeg != null || cogDeg != null;
  const ring = selected
    ? `<circle cx="14" cy="14" r="13" fill="none" stroke="${color}" stroke-width="2" stroke-opacity="0.9"/>`
    : `<circle cx="14" cy="14" r="11" fill="${color}" fill-opacity="0.18" stroke="${color}" stroke-width="1" stroke-opacity="0.5"/>`;
  const arrow = hasDir
