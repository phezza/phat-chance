...
function Sidebar({
  ...
  return (
    ...
    >
      <div
        className={cn(
          "h-[61px] flex items-center border-b border-white/8 flex-shrink-0 px-3",
      ...
      <div className="flex-shrink-0 border-t border-white/8">
        ...
        <button
          onClick={onToggle}
          className={cn(
            "w-full flex items-center gap-2 px-3 py-2.5 text-white/30 hover:text-white/60 hover:bg-white/5 transition-colors border-t border-white/8 text-xs font-mono",
...
function MobileHeader({ onOpen }: { onOpen: () => void }) {
  return (
    <header className="h-[61px] flex items-center gap-3 px-4 border-b border-white/8 bg-black/30 backdrop-blur-sm flex-shrink-0 lg:hidden">
      <button
        onClick={onOpen}
        className="w-9 h-9 rounded-lg flex items-center justify-center text-white/50 hover:text-white hover:bg-white/10 border border-white/10 transition-colors flex-shrink-0"
...
function MobileDrawer({ open, onClose }: { open: boolean; onClose: () => void }) {
  return (
    <>
      ...
      >
        <div className="h-[61px] flex items-center justify-between px-4 border-b border-white/10 flex-shrink-0">
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 rounded-lg bg-cyan-500/20 border border-cyan-500/30 flex items-center justify-center">
              <Anchor className="w-4 h-4 text-cyan-400" />
            </div>
            <span className="font-bold text-white tracking-wide text-sm">Phat Chance</span>
          </div>
          <button
            onClick={onClose}
